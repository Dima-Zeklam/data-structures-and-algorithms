...
const toLastNames = people => {
  // Solution code here...
let newArr = people.reduce((ele,val,index) =>{
  ele[index]=  val.firstName + ' ' + val.lastName;  
  return ele;
},[]);
return newArr;
}
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let inputArray = arr.reduce((result,Input,index)=>{
    result = result + 1;
    return result;
  },0) 
  return inputArray;
...
const reversedString = (str) => {
  ...
  for(let i = 0;i<str.length;i++){
   ...
   let newArray = strArr.reduce((acc,val,i)=>{
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let newArr = arr.reduce((acc,ele,index)=>{
