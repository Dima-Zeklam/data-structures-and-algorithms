...
function transformToLis(obj) {
  ...
  for (let i = 0; i < keyArr.length; i++) {
    finalArr[i] = `<li>${keyArr[i]}: ${valueArr[i]}</li>`;
  }
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let array = input.map(item=>{
    ...
    newArr = item.filter(ele => {
      if(ele%5===0 && typeof(ele) === 'number'){ 
      return ele;
      }
...
if(ele.gender === 'male' || ele.gender === 'female')
    dataArr.push(ele.name);

  })
  // console.log(dataArr.join(' and '));
  return dataArr.join(' and ');
};
...
let name='';
  data.forEach(ele=>{
      dataArr.push(ele.height);
  })
 let sorArr= dataArr.sort((a,b)=>{
    if(a>b)
    return -1;
    else
    if(b>a)
